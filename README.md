...
*Kfold.py*: This code extends the main model from `main_model.py` by incorporating k-fold cross-validation to ensure robust model evaluation. The CNN architecture, along with early stopping and other components, is based on `main_model.py`. To implement k-fold cross-validation, StratifiedKFold was used to split the dataset, with each fold undergoing training and validation. Metrics such as accuracy, precision, recall, and F1 score are calculated for each fold to provide a comprehensive evaluation of model performance.
...
**Training and Evaluation**
   ...
   To apply the K-fold cross validation after bias mitigation
