'''
THIS CODE IS TO FIX THE BIAS.
Some modification were made to the main model such as, data augmentation, regularization techniques, and balanced sampling to ensure fair and unbiased model performance.
...
for fold, (train_idx, test_idx) in enumerate(skf.split(dataset.samples, dataset.targets)):
    ...
    # class_counts = np.bincount([dataset.targets[i] for i in train_sub_idx])
    # class_weights = 1. / class_counts
    # sample_weights = np.array([class_weights[dataset.targets[i]] for i in train_sub_idx])
    ...
    torch.save(kfold_model, f'k_model_fold_bias_{fold + 1}.pth') #saves model state for current fold 
